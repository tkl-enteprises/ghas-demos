...
In this workshop the "tokens" are fake — there's nothing to revoke. In a real org, a custom-pattern alert kicks off the same incident response as any other secret leak (see `lessons/07-secret-protection-secret-scanning/solution.md` for the general runbook), with one extra step at the front:
