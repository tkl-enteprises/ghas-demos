...
7. **Review the GitHub Actions alert.** [Open the prefiltered Lesson 05 alert list](https://github.com/tkl-enteprises/ghas-demos/security/code-scanning?query=is%3Aopen+branch%3Amain+path%3A.github%2Fworkflows%2Flesson-05-vulnerable.yml), or enter the exact filter `is:open branch:main path:.github/workflows/lesson-05-vulnerable.yml`. The `path:` qualifier matches a file; `path:.github/workflows/` is not a directory-prefix search and returns no results. Workflow files must live under the repository-root `.github/workflows/` directory, so `path:lessons/05-code-security-actions` cannot return a GitHub Actions alert. The two critical `.txt` fixture findings appear only if you completed the disposable-fork exercise above. Useful rule IDs include:
