...
3. **Enable secret scanning + push protection.** Repo → `Settings → Code security → Secret scanning`. Turn on *Secret scanning*, *Push protection*, and *Push protection for contributors* if your org policy allows. Push protection is what makes lesson 4 land — without it, secrets only get flagged after the fact.
...
6. **Publish the two custom secret patterns for lesson 5.** Repo → `Settings → Code security → Secret scanning → Custom patterns → New pattern`. Lesson 5's `.github/secret_scanning.yml` was deliberately removed because that file only supports path exclusions — *not* custom-pattern definitions. Add these two by hand:
   ...
   Tick *Push protection* on each if you want lesson 5's optional push-block step to work. See [lesson 5 README](lessons/05-custom-secret-patterns/README.md) for screenshots of the preflight.
...
| Dependabot alerts + security updates | enabled | Powers lesson 6 |
...
| Custom secret patterns (lesson 5) | UI-only, not in source | See lesson 5 README + preflight step 6 |
...
| 2 | Copilot Autofix | short | One-click suggestion → review → commit. Fast and visual. |
| 3 | Custom CodeQL Queries | long | The most technical lesson. Skip on non-engineering audiences. |
| 4 | Secret Scanning + Push Protection | short | Live `git push` of a fake key is the whole demo — keep it tight. |
| 5 | Custom Secret Patterns | medium | Org admins love this; ICs less so. Match to your audience. |
| 6 | Dependabot / Supply Chain (+ Malware bonus) | medium | Walk through both alerts *and* the auto-generated PRs. End with the Malware tab tour. |
...
| 9 | Code Quality (bonus / preview) | short | Same engine, different queries. Skip on pure-security audiences. |
...
- ❌ **"Push protection let me push my secret."** Push protection only blocks **patterns GitHub recognizes** by default. The fake Azure connection string in lesson 4 matches a partner pattern; randomly chosen strings will not. If a demo "fails," check the pattern, not the feature.
- ❌ **"Lesson 5 alerts aren't appearing."** Custom patterns are not version-controlled — they live in repo *Settings*. If you skipped step 6 of the preflight, the two custom patterns aren't published, and the demo files in lesson 5 will sit silent regardless of how long you wait.
...
- ❌ **"Several 'Dependabot Updates' workflow runs show as failure."** Those red runs are Dependabot's *internal* rebase/recompute jobs, not the workshop's CI. They fire when Dependabot can't compute a clean update graph for a manifest (typically because a transitive dep also needs to move, or two PRs touch the same lockfile). They do **not** block PR creation — the seven pip-dep PRs in lesson 6 still appear, still surface their CVE annotations, and still merge correctly. Safe to ignore; do not panic-debug them mid-session.
- 🎤 **Azure-vs-AWS optics — for Microsoft FTEs delivering to customers, prefer Azure / Contoso fixtures.** Lesson 4's primary fixture is an Azure storage connection string (`AccountKey=FAKEDEMO…==`) and lesson 5's custom patterns use the `Contoso` prefix on purpose: every Markdown file, every code sample, every screenshot is potential customer-facing material — one screenshot of an `AKIA…`-shaped value on Slack from an excited customer is a bad day. GHAS's secret-scanning coverage is genuinely vendor-neutral (200+ partner patterns including AWS, GCP, Slack, OpenAI, Stripe, …) so the demo translates 1:1 to any cloud; we just lead with Azure-shaped fixtures in source. Vendor-neutrality lives in the *prose* (e.g. solution.md's incident-response runbook calls out Azure / AWS / GCP / Stripe / GitHub side-by-side).

### Push protection bypass — mitigations for live demos

The verification run logged in [`docs/screenshots/push-protection-block.txt`](docs/screenshots/push-protection-block.txt) shows the failure mode that surprises facilitators most: **a fresh partner-pattern canary pushed by an admin pushes through cleanly with `EXIT_CODE: 0` and no block prompt**, even with push protection switched on in repo settings. (The committed transcript captures the workshop's previous AWS-first fixtures — the failure mode is pattern-agnostic; the same admin-bypass behaviour applies to the current Azure-first lesson and to lesson 5's `CONTOSO-API-…` custom pattern.)
...
### 9. Code Quality (bonus / preview)
[Code Quality](https://github.com/tkl-enteprises/ghas-demos/security/quality) is a *separate* GitHub product that runs on the same CodeQL engine — but with a maintainability + reliability query pack instead of security queries. Same engine, different queries. It's a Preview feature, billed as Action minutes (not GHAS seats), and push protection / branch rulesets do **not** apply to its findings (advisory only). The teaching contrast on this repo is sharp: 132 security findings vs 0 standard quality findings — same code, different verdict.
...
| --- | --- | --- | --- |
| **#21** | `py/template-injection` (SSTI) | `lessons/02-copilot-autofix/insecure_login.py` | High severity, dataflow path is short, Copilot's fix typically introduces `escape()` / autoescape config — easy to read. |
| **#28** | `py/weak-cryptographic-algorithm` | `lessons/02-copilot-autofix/insecure_login.py` | Smallest possible fix surface (one-line MD5 → SHA-256 swap). Use this when you have ≤ 5 minutes for the lesson. |
...
A non-zero number means the seat pool exists; `none` (or a 404) means Copilot isn't enabled on the org and lesson 02 should be **swapped for an extra pass through lesson 1** (CodeQL alert triage UI). Document this in the agenda you send attendees so nobody opens a Copilot tab and finds it empty.
...
| [`docs/screenshots/04-push-protection-settings.png`](docs/screenshots/04-push-protection-settings.png) | 4 — Secret Scanning + Push Protection | Repo `Settings → Code security → Secret scanning` showing the push-protection toggle on. |
| [`docs/screenshots/05-secret-scanning-default-empty.png`](docs/screenshots/05-secret-scanning-default-empty.png) | 4 — Secret Scanning + Push Protection | `Security → Secret scanning` **Default** tab showing "No secrets found" — illustrates AI suppression on committed canaries. |
| [`docs/screenshots/05-secret-scanning-generic-ai.png`](docs/screenshots/05-secret-scanning-generic-ai.png) | 4 + 5 — Secret scanning / Custom patterns | `Security → Secret scanning` **Generic** tab — the AI classifier firing on `hunter2_FAKE_*`-style password assignments. |
| [`docs/screenshots/06-dependabot-alerts.png`](docs/screenshots/06-dependabot-alerts.png) | 6 — Dependabot / Supply Chain | `Security → Dependabot` alerts list across Flask, Jinja2, Werkzeug, urllib3, requests, PyYAML, cryptography. |
| [`docs/screenshots/06-dependabot-prs-list.png`](docs/screenshots/06-dependabot-prs-list.png) | 6 — Dependabot / Supply Chain | Pull requests tab filtered to `app/dependabot` — the seven open pip security-update PRs. |
| [`docs/screenshots/06-dependabot-pr-detail.png`](docs/screenshots/06-dependabot-pr-detail.png) | 6 — Dependabot / Supply Chain | A Flask-bump security-update PR with CVE annotation, release notes, and compatibility score. |
| [`docs/screenshots/03-custom-codeql-rule-list.png`](docs/screenshots/03-custom-codeql-rule-list.png) | 3 — Custom CodeQL Queries | Code-scanning alerts filtered to rule `py/tkl/hardcoded-debug-true` — proves precision (`bypass.py` absent) and recall (`target.py` present). |
| [`docs/screenshots/03-custom-codeql-alert-detail.png`](docs/screenshots/03-custom-codeql-alert-detail.png) | 3 — Custom CodeQL Queries | Alert detail for the custom-query finding — `@id`, `@kind`, source location. |
...
| [`docs/screenshots/push-protection-block.txt`](docs/screenshots/push-protection-block.txt) | 4 — Secret Scanning + Push Protection | **Historical artifact** — verbatim terminal capture from the 2026-05-06 verification run against the workshop's *previous* AWS-first fixtures. **OBSERVED:** push protection did **not** block a fresh `AKIA…` canary on the ephemeral branch (`EXIT_CODE: 0`). The lesson's current Azure-first / Contoso fixtures behave the same way for the same root cause (admin bypass list + validity-check deprioritisation); we keep the historical evidence intact rather than re-recording. Re-verify enforcement before each delivery — see *Common attendee gotchas* and *Push protection bypass — mitigations for live demos* above for context. |

Gaps still worth capturing live in your tenant for a follow-up deck:

- 📸 Lesson 2 — the **Copilot Autofix** suggestion diff (before / after acceptance) on the `py/sql-injection` alert in `insecure_login.py`.
...
→ See [lesson 9](lessons/09-code-quality/) for the full walkthrough; key talking points below for fast reference.

- **Same engine, different queries.** [Code Quality](https://github.com/tkl-enteprises/ghas-demos/security/quality) is a *separate* GitHub product from GHAS, but it runs on the same CodeQL engine. Maintainability + reliability query pack instead of taint / SQLi / SSRF.
- **Status:** **Preview** as of late 2025. UI may change before GA.
- **Billing:** charged as **Action minutes** (NOT GHAS seats) — important for the buying conversation.
- **Advisory only.** Push protection / branch rulesets do **not** apply to quality findings; you can't gate merges on them via the standard ruleset path.
- **Teaching contrast.** This repo: **132 security findings** vs **0 standard quality findings** + 5 AI findings on `FACILITATOR.md`. Same engine, same code, different verdict — that's the headline.
- **When to skip.** CISO briefings or pure-security audiences — drop it. The 60-min agenda skips it; the half-day agenda includes it as a 5–10 min interlude after lesson 3.
...
### 60-minute executive overview (lessons 1, 4, 6, 8)

For C-suite, security leadership, or non-technical buyers. Skips lessons 2, 3, 5, 7 — those land flatter without an engineering audience. Goal: leave the room knowing GHAS catches code bugs, leaked secrets, and vulnerable dependencies, and that there's a single org-level cockpit for all three.
...
| 0:15–0:25 | 4 — Secret Scanning | Live `git push` of a fresh `CONTOSO-API-…` line on a workshop branch (custom pattern from lesson 5 — see *Push protection bypass* mitigations above). | What's our current MTTR on a leaked production secret? |
| 0:25–0:35 | 6 — Dependabot | Show the seven open pip security-update PRs. Open one and walk the CVE annotation. Tour the empty Malware tab. | Who owns merging these today? How long do they sit? |
...
**Total: 60 min.** ⚠️ The lesson-4 demo *must* use the lesson-05 custom pattern — partner-pattern push protection silently bypasses for admins (see mitigation note above).

### 2-hour developer enablement (lessons 1, 2, 4, 5, 6, 7, 8)

For platform / appsec / senior dev audiences who own day-to-day triage. Covers seven of eight lessons; deliberately skips lesson 3 (custom CodeQL queries) — that's its own half-day topic and you'll lose the room if you try to compress it into 15 minutes. Goal: every attendee leaves able to triage an alert, push past a custom-pattern block, and read a SARIF upload.
...
| 0:25–0:35 | 2 — Copilot Autofix | Generate-fix on alert #21 or #28. Edit-then-accept the diff. | When *not* to trust Autofix? What's your code-review SLA? |
| 0:35–0:50 | 4 — Secret Scanning | Live custom-pattern push protection block (see lesson 05). Show the secret-scanning Default + Generic tabs. | Why doesn't the Azure storage canary block? (Validity checks + bypass list — same root cause that suppresses the historical `AKIA…` capture.) |
| 0:50–1:00 | 5 — Custom Patterns | Settings → Secret scanning → Custom patterns. Walk the two `Contoso …` patterns (pre-published per pre-flight step 6). Push a fresh `contoso_demo_…` and watch it block. | What internal token shapes belong here? (Badge IDs, internal API keys, vendor IDs.) |
| 1:00–1:10 | — | ☕ Break | — |
| 1:10–1:25 | 6 — Dependabot | PR list filter `app/dependabot`. Walk one PR; mention the red "Dependabot Updates" runs are internal rebase jobs (gotchas). Tour Malware tab. | Auto-merge policy: green CI + Dependabot? Where's the line? |
...
### Half-day deep dive (all 8 lessons + lesson 9 bonus)
...
| 0:40–0:55 | 2 — Copilot Autofix | Autofix on alert #21. Edit-then-accept. Try a second alert and dismiss-with-reason. | Where Autofix earns its keep; where it doesn't. |
| 0:55–1:40 | 3 — Custom CodeQL | Walk `lessons/03-custom-codeql-queries/queries/` — read `py/tkl/hardcoded-debug-true.ql`. Run locally with `codeql database analyze`. Show the alert appearing on the next PR. | Authoring custom queries: when is this the right tool vs. a lint rule? |
| 1:40–1:50 | 9 — Code Quality (bonus) | `Security → Code quality → Standard findings` (Excellent / 0 findings). Compare with Code scanning side-by-side. AI findings on `FACILITATOR.md`. | Same engine, different queries — would your team gate merges on quality? |
| 1:50–2:05 | — | ☕ Break | — |
| 2:05–2:25 | 4 — Secret Scanning | Custom-pattern push block (lesson 05). Show AI-suppressed Azure storage canary in `config.py`. Walk validity-check flag in alert detail. | Threat model: leaked secrets in commits vs CI logs vs container images. |
| 2:25–2:45 | 5 — Custom Patterns | Walk the two `Contoso …` patterns; push-block a fresh `contoso_demo_…`; show the matching alert. | Pattern hygiene: pre/post-match heuristics, false-positive rate. |
| 2:45–3:10 | 6 — Dependabot (+ Malware) | Alerts list → PR list → PR detail. Show CVSS + EPSS + compatibility score. Tour Malware tab + real-world examples. | Dependabot vs reachability: where do we draw the line? |
| 3:10–3:25 | — | ☕ Break | — |
| 3:25–3:45 | 7 — SARIF | `Bandit-SARIF` workflow run → upload-sarif step → unified Code-scanning view. Show how Bandit findings interleave with CodeQL findings. | Tool consolidation: which scanners stay, which go? |
| 3:45–3:55 | 8 — Security Overview | Org **Risk**, **Coverage**, **Configurations**, **Campaigns** (if licensed). Walk a Configurations apply. | 90-day rollout plan: who, what, when. |
| 3:55–4:00 | — | Wrap, action items, follow-up resources | — |

**Total: 240 min** with two 15-minute breaks. Lesson 3 gets the largest slot (45 min) because authoring + running a custom query takes real time and is the most technical content of the day. Lesson 9 is a 5–10 minute interlude after lesson 3 — same engine, different queries, same UI shell.
