...
8. **Prepare lesson 11 separately if you plan to run it live.** AI-powered security detections require CodeQL default setup, but this repository uses advanced setup for custom queries and Actions analysis. Use a disposable organization-owned copy with the required Code Security, Copilot, AI-credit, and policy settings; otherwise use lesson 11's source-review fallback. Do not replace this repository's advanced setup during a workshop.
...
| AI-powered security detections (lesson 11) | source-review fallback in this repo | Live preview requires a separate eligible repository with CodeQL default setup |
...
| 9 | Code Quality (bonus / public preview) | short | Same engine, different queries. Skip on pure-security audiences. |
| 10 | CodeQL for GitHub Actions | medium | Review trust boundaries first; use the inert `.txt` fixtures unless working in a disposable copy. |
| 11 | AI-powered security detections (optional / public preview) | short | Use a separately configured repository or the documented source-review fallback. |
...
### 9. Code Quality (bonus / public preview)
...
### 11. AI-powered security detections (optional / public preview)
...
| [`docs/screenshots/05-secret-scanning-default-empty.png`](docs/screenshots/05-secret-scanning-default-empty.png) | 4 — Secret Scanning + Push Protection | `Security and quality → Secret scanning` **Default** tab showing "No secrets found" — illustrates AI suppression on committed canaries. |
| [`docs/screenshots/05-secret-scanning-generic-ai.png`](docs/screenshots/05-secret-scanning-generic-ai.png) | 4 + 5 — Secret scanning / Custom patterns | `Security and quality → Secret scanning` **Generic** tab — the AI classifier firing on `hunter2_FAKE_*`-style password assignments. |
| [`docs/screenshots/06-dependabot-alerts.png`](docs/screenshots/06-dependabot-alerts.png) | 6 — Dependabot / Supply Chain | `Security and quality → Dependabot` alerts list across Flask, Jinja2, Werkzeug, urllib3, requests, PyYAML, cryptography. |
...
For platform / appsec / senior dev audiences who own day-to-day triage. Covers seven foundational lessons; deliberately skips lesson 3 (custom CodeQL queries) and the optional/advanced lessons — they'll crowd out hands-on triage in this format. Goal: every attendee leaves able to triage an alert, push past a custom-pattern block, and read a SARIF upload.
...
| 0:15–0:35 | 1 — CodeQL | Full triage flow on alert #21: detail → dataflow → close as fixed via PR. | When CodeQL is wrong: investigation flow + dismissal reasons. |
| 0:35–0:45 | 2 — Copilot Autofix | Autofix on alert #21. Edit-then-accept. | Where Autofix earns its keep; where it doesn't. |
| 0:45–1:15 | 3 — Custom CodeQL | Read and run `py/tkl/hardcoded-debug-true.ql`; show the resulting alert. | Custom query vs. lint rule? |
| 1:15–1:30 | 10 — Actions scanning | Compare the inert vulnerable and remediated workflow fixtures; show **Analyze (actions)**. | Where do workflow privileges cross trust boundaries? |
| 1:30–1:40 | 9 — Code Quality (bonus) | Compare Standard and AI quality findings with Code scanning. | Would your team gate merges on quality? |
| 1:40–1:55 | — | ☕ Break | — |
| 1:55–2:10 | 4 — Secret Scanning | Custom-pattern push block; show the validity-check distinction. | Leaks in commits vs CI logs vs images. |
| 2:10–2:25 | 5 — Custom Patterns | Walk the Contoso patterns and push-block a fresh demo value. | Pattern hygiene and false positives. |
| 2:25–2:45 | 6 — Dependabot (+ Malware) | Alerts → update PR → CVSS/EPSS; tour Malware. | Dependabot vs reachability. |
| 2:45–3:00 | 7 — SARIF | Bandit upload → unified code-scanning view. | Which scanners stay? |
| 3:00–3:15 | — | ☕ Break | — |
| 3:15–3:35 | 11 — AI security detections | In a disposable eligible copy, inspect AI-labeled PR findings; otherwise run the source-review fallback. | How should advisory AI findings be governed? |
| 3:35–3:50 | 8 — Security Overview | Org **Risk**, **Coverage**, **Configurations**, and **Campaigns** if available. | 90-day rollout plan. |
| 3:50–4:00 | — | Wrap, action items, follow-up resources | — |

**Total: 240 min** with two 15-minute breaks. Lesson 3 gets the largest slot because authoring and running a custom query is the most technical content. Lesson 11's live path assumes its disposable default-setup repository passed preflight; otherwise use the source-review fallback without changing this repository.
